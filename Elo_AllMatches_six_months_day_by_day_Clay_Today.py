...
def Elo(date_six_months_ago, date_today, date_today_formatted):
    data = pd.read_sql_query(
        "Select distinct Surface,Date,Sex,Player_1 as Winner, Player_2 as Loser, Player_1_Odds as Winner_Odds, Player_2_Odds as Loser_Odds FROM AllMatches where surface like 'Clay' and tournament not like '%UK Pro%'  and tournament not like '%UTR%' and tournament not like '%Davis%' and date >='{}' and date <= '{}' ".format(
            date_six_months_ago, date_today
        ),
        con=devengine,
    )
    data2 = pd.read_sql_query(
        "Select distinct Surface,Date,Sex,Player_1 as Winner, Player_2 as Loser, Player_1_Odds as Winner_Odds, Player_2_Odds as Loser_Odds,Resulted,Time FROM TodaysMatches where surface like 'Clay' and tournament not like '%UK Pro%'  and tournament not like '%UTR%' and tournament not like '%Davis%' ",
        con=devengine,
    )
    ...
    data.to_sql(
        "Elo_AllMatches_Daily_Clay_Today",
        con=devengine,
        if_exists="replace",
        index=False,
    )
...
def clay():
    ...
    fav_dog = "dog"
    id = "6monthsdog"
    query = "Select * From Elo_AllMatches_Daily_Clay_Today where ((Elo_Dog_Odds BETWEEN 2 AND 2.1) OR (Elo_Dog_Odds BETWEEN 2.5 AND 2.6) \
        OR (Elo_Dog_Odds BETWEEN 3.1 AND 3.3) OR (Elo_Dog_Odds BETWEEN 3.4 AND 3.5) OR (Elo_Dog_Odds BETWEEN 3.6 AND 3.7)) and sex = 'Mens'"
    get_data(month_year, past, query, fav_dog, id)
    ...
    fav_dog = "dog"
    id = "3monthsdog"
    query = "Select * From Elo_AllMatches_Daily_Clay_Today where ((Elo_Dog_Odds BETWEEN 2 AND 2.1) OR (Elo_Dog_Odds BETWEEN 2.5 AND 2.6) OR (Elo_Dog_Odds BETWEEN 3.1 AND 3.3) \
        OR (Elo_Dog_Odds BETWEEN 3.4 AND 3.5) OR (Elo_Dog_Odds BETWEEN 3.6 AND 3.7) OR (Elo_Dog_Odds BETWEEN 3.8 AND 3.9)) and sex = 'Mens'"
    get_data(month_year, past, query, fav_dog, id)
    ...
    fav_dog = "dog"
    id = "3monthsdog"
    query = "Select * From Elo_AllMatches_Daily_Clay_Today where ((Elo_Dog_Odds BETWEEN 1.9 AND 2.1) OR (Elo_Dog_Odds BETWEEN 2.2 AND 2.5) \
        ) and sex = 'Womens'"
    get_data(month_year, past, query, fav_dog, id)
