...
def Main(url, current_date, suffix, check):
    ...
    for i, item in enumerate(tournament_idx_lst[:-1]):
        ...
        if 'Futures' not in tournament_name and (\
            'ITF'  in tournament_name or \
            #'UTR'  in tournament_name or \
            'Pro Series'  in tournament_name):            
        ...
        for i, date in value.items():
            for match in date:
                ...
                if ValidName(player2) == True and ValidName(player1)==True:
                    Elo_player1, Elo_player2,Fav,elo_diff,Prob,Odds,ActualOdds,OddsClay,OddsHard=GetElo(player1,player2,player1odds,player2odds)
                    ...
                    table = [['Date','Sex', 'Tournament', 'Player 1', 'Player 1 Elo', 'Player 2',\
                         'Player 2 Elo','Elo Favourite','Elo Difference','Elo Probability','Estimated Odds',\
                             'Actual Odds', 'Estimated Odds Clay','Estimated Odds Hard','Surface'], [datefilename,
                            suffix.replace('_', ''), key, player1, Elo_player1, player2, Elo_player2,Fav,elo_diff,Prob,Odds,ActualOdds,OddsClay,OddsHard,Surface]]
                    ...
                    new_df.to_sql("Test",con=devengine,if_exists='append',index=False)
...
for x in range(1,3):
    ...
    Filterdf=pd.read_sql_query("Select * From Test",con=devengine)
    ...
    bra3=bra3.drop(columns=["Player 1 Elo", "Player 2 Elo","Elo Difference"])
    #bra3["Estimated Odds"]=bra3["Estimated Odds"].astype(float)
    #bra3["Actual Odds"]=bra3["Actual Odds"].astype(float)
    bra3["Estimated Odds Clay"]=bra3["Estimated Odds Clay"].astype(float)
    ...
    filter=(bra3['Estimated Odds'].lt(2)|bra3['Estimated Odds Clay'].lt(2)|bra3['Estimated Odds Hard'].lt(2))
    bra3=bra3[filter]
    ...
    bra4.to_sql("Bets_ITF",con=devengine,index=False,if_exists="append")