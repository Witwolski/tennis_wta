...
def Main(url, current_date, suffix, check):
    ...
    df = pd.DataFrame(columns=['Sex', 'Tournament', 'Player 1', 'Player 1 Elo', 'Player 2', 'Player 2 Elo','Elo Favourite','Elo Difference','Elo Probability','Estimated Odds','Actual Odds','Estimated Odds grass'])
        ...
        for i, date in value.items():
            for match in date:
                ...
                if ValidName(player2) == True and ValidName(player1)==True:
                    ...
                    new_df.to_sql("Test_Yesterday_grass",con=devengine,if_exists='append',index=False)

#for x in range(81,90):
for x in range(1,2):
    
    connection.execute('Delete FROM Test_Yesterday_grass')
    ...
    Filterdf=pd.read_sql_query("Select DISTINCT * From Test_yesterday_grass",con=devengine)
    ...
    bra4.to_sql("bets_yesterday_grass",con=devengine,index=False,if_exists="append")