...
def elo_mbm(sex, mask, after_date, grt):
    query = "Select * FROM Elo_AllMatches  where sex like '{}' and date  >= Convert(datetime, '{}' ) and surface = 'Hard'".format(
        sex, after_date
    )
...
combine_results.to_excel("Analysis_Hard.xlsx", index=False)
