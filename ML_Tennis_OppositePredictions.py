...
def ML(tomorrow, month_ago, six_months_ago, yesterday):
    dataset = pd.read_sql_query(
        "Select Winner, Elo_Fav,Elo_Fav_Odds, Elo_Dog_Odds, Elo_Winner, Elo_Loser FROM Elo_AllMatches where  date < '{}' and WinnerTotal > 50 and LoserTotal > 50 and Elo_Fav_Odds > 1.8".format(
            month_ago
        ),
        con=devengine,
    )
    ...
    prediction = pd.read_sql_query(
        "Select  Winner,Loser,Elo_Fav, Elo_Fav_Odds ,Elo_Dog_Odds, Elo_Winner, Elo_Loser FROM Elo_AllMatches where  date > '{}' and date < '{}' and WinnerTotal > 50 and LoserTotal > 50 and Elo_Fav_Odds > 1.8".format(
            month_ago, tomorrow
        ),
        con=devengine,
    )
    ...
    prediction1 = pd.read_sql_query(
        "Select Date,Winner,Loser,Elo_Fav, Elo_Fav_Odds, Elo_Dog_Odds, Elo_Winner, Elo_Loser  FROM Elo_AllMatches where  date > '{}' and date < '{}' and WinnerTotal > 50 and LoserTotal > 50 and Elo_Fav_Odds > 1.8".format(
            month_ago, tomorrow
        ),
        con=devengine,
    )
...
db_table = "Predictions_Past_two_four_odds_rounded_opp"
...
for day in range(0, 100):
    ...
    if x.empty == False:
        ...
        if x.empty == False:
            ...
            x.to_sql(
                "Predictions_Past_two_four_odds_rounded_opp",
                con=devengine,
                if_exists="append",
                index=False,
            )
